#!/bin/bash
# Запускает cash-service локально, направляя HTTP-зависимости в WireMock.
#
# Предварительно:
...
#   3. Собери jar:
#        /Users/maksim/apps/apache-maven-3.9.9/bin/mvn -pl cash-service package -DskipTests

set -e

JAR=$(ls /Users/maksim/Desktop/All/bankapp/cash-service/target/cash-service-*.jar 2>/dev/null | head -1)

if [ -z "$JAR" ]; then
  echo "JAR не найден. Сначала запусти: mvn -pl cash-service package -DskipTests"
  exit 1
fi

echo "Запускаю cash-service -> accounts-mock:8091, blocker-mock:8092"

ACCOUNTS_SERVICE_URL=http://localhost:8091 \
BLOCKER_SERVICE_URL=http://localhost:8092 \
NOTIFICATIONS_SERVICE_URL=http://localhost:8091 \
