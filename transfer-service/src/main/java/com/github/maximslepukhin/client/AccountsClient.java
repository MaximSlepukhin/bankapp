...
public interface AccountsClient {
    ...
    @GetMapping("/api/accounts/{login}/accounts/currencies")
    List<String> getCurrencies(@PathVariable("login") String login);

    @PostMapping("/api/accounts/{login}/accounts/{currency}/debit")
    void debit(@PathVariable("login") String login,
               @PathVariable("currency") String currency,
               @RequestParam BigDecimal amount);

    @PostMapping("/api/accounts/{login}/accounts/{currency}/deposit")
